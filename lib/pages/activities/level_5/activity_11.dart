...
// Import your updated Activity 11 pages
...
class Activity11Page extends StatefulWidget {
  final VoidCallback? onCompleted; // ✅ Callback to update TaskListPage

  const Activity11Page({super.key, this.onCompleted});
...
class _Activity11PageState extends State<Activity11Page> {
  ...
  Future<void> _goToPage(int newPage) async {
    ...
    /// ✅ Finish activity when reaching last page
    if (newPage == _pages.length - 1 &&
        _pageCompleted.every((completed) => completed)) {
      await _saveTaskStatus("Task 11", "Completed");
