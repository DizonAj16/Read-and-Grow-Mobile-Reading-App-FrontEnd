...
class _AdminViewTeachersPageState extends State<AdminViewTeachersPage> {
      ...
      children: [
                                                    ...
                                                    child: Column(
                                                      ...
                                                      children: [
                                                        ...
                                                        Row(
                                                          mainAxisAlignment: MainAxisAlignment.center,
                                                          children: [
                                                            _infoColumn("Email", teacher.email ?? "N/A", context),
                                                            SizedBox(width: 48),
                                                            _infoColumn("Username", teacher.username ?? "N/A", context),
                                                          ],
                                                        ),
                                                        SizedBox(height: 18),
                                                        Row(
                                                          mainAxisAlignment: MainAxisAlignment.center,
                                                          children: [
                                                            _infoColumn("Position", teacher.position ?? "N/A", context),
                                                            SizedBox(width: 48),
                                                            _infoColumn("ID", teacher.id?.toString() ?? "N/A", context),
                                                          ],
                                                        ),
                                                        SizedBox(height: 24),
                                                      ],
                                      ...
                                      if (confirm == true) {
                                        try {
                                          if (teacher.userId != null) {
                                            final response = await ApiService.deleteUser(teacher.userId);
                                            ...
                                            ScaffoldMessenger.of(context).showSnackBar(
                                              SnackBar(
                                                content: Text('Teacher user ID is missing'),
                                                backgroundColor: Colors.red,
                                              ),
                                            );
                                          }
                                          ...
                                          ScaffoldMessenger.of(context).showSnackBar(
                                            SnackBar(
                                              content: Text('Error deleting teacher'),
                                              backgroundColor: Colors.red,
                                            ),
                                          );
                                        }
        ...
        Row(
          ...
          children: [
            ...
            Text('Page ${_allTeachers.isEmpty ? 0 : _currentPage + 1} of $totalPages'),
            IconButton(
              icon: Icon(Icons.arrow_circle_right_sharp, size: 40),
              onPressed: (_currentPage + 1) * _pageSize < _allTeachers.length ? _goToNextPage : null,
            ),
          ],
        ),
      ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("All Teachers", style: TextStyle(color: const Color.fromARGB(255, 255, 255, 255))),
  ...
  Widget _infoColumn(String label, String value, BuildContext context, {double fontSize = 15, double valueFontSize = 15}) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
